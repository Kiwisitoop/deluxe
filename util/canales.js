...
module.exports = {
  nombre: "canales",
  alias: ["channels", "infochannels"],
  descripcion: "Este comando sirve para mostrar los canales de forma estructurada.",
  run: async (client, message, args) => {
    ...
    embed.setTitle(`Canales y categorías de ${message.guild.name}`)
    embed.setDescription(texto.length > 0 ? "```"+texto+"```" : "No datos.")
    embed.setThumbnail(message.guild.iconURL())
    return message.channel.send(embed).catch(e => message.channel.send("Hay demasiados canales y categorías en este servidor.")) //Enviamos el embed con la estructura que construimos en la variable "texto"
